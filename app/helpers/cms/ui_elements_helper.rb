module Cms
  module UiElementsHelper
    ...
    def save_and_publish_button(block, content_type, options={})
      defaults={:label=>"Save And Publish"}
      options=defaults.merge(options)
      if current_user.able_to?(:publish_content) && block.publishable?
        html = %Q{<button type="submit" name="#{content_type.content_block_type.singularize}[publish_on_save]" value="true" class="submit" tabindex="#{next_tabindex}"><span>#{options[:label]}</span></button>}
